...
export class SnapshotManager {
    ...
    schema;
    serializer;
    constructor(runtime, schema, serializer) {
    ...
    async createSnapshot(containerId) {
        try {
            const tree = SnapshotBuilder.build(containerId, this.runtime, this.schema, this.serializer);
            // 将整棵快照树编码为二进制数据
            const binaryData = this.serializer.encode(serializableTree(tree));
            return {
                containerId,
            ...
            throw new SnapshotError("创建快照失败", ContainerErrorCode.SnapshotFailed, { containerId }, error instanceof Error ? error : new Error(String(error)));
        }
    }
    /**
     * 从快照恢复容器状态。
    ...
    async restoreFromSnapshot(snapshot) {
        try {
            ...
            throw new SnapshotError("从快照恢复失败", ContainerErrorCode.SnapshotRestoreFailed, { containerId: snapshot.containerId }, error instanceof Error ? error : new Error(String(error)));
        }
    }
}
/**
 ...
 */
function serializableTree(tree) {
    const trees = {};
