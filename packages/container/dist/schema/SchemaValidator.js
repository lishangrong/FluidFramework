...
export class SchemaValidator {
     ...
     */
    static validate(schema, registeredTypes) {
        // 验证初始对象中的类型
        for (const [name, type] of schema.initialObjects) {
            if (!registeredTypes.has(type)) {
                throw new SchemaError(`初始对象 "${name}" 的类型 "${type}" 未注册`, ContainerErrorCode.UnknownDdsType, { name, type });
            }
        }
        // 验证动态对象类型
        for (const type of schema.dynamicObjectTypes) {
            if (!registeredTypes.has(type)) {
                throw new SchemaError(`动态对象类型 "${type}" 未注册`, ContainerErrorCode.UnknownDdsType, { type });
            }
        }
    }
}
