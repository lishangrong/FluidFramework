...
export class ContainerSchema {
     ...
     * 检查指定类型是否可动态创建。
     */
    isDynamicTypeAllowed(type) {
        return this.dynamicObjectTypes.includes(type);
    }
    /**
     * 获取初始对象的类型。
     */
    getInitialObjectType(name) {
    ...
    validateSchema(schema) {
        if (!schema.initialObjects || typeof schema.initialObjects !== "object") {
            throw new SchemaError("initialObjects 必须是非空对象", ContainerErrorCode.InvalidSchema);
        }
        const keys = Object.keys(schema.initialObjects);
        if (keys.length === 0) {
            throw new SchemaError("initialObjects 必须包含至少一个条目", ContainerErrorCode.InvalidSchema);
        }
        for (const [key, type] of Object.entries(schema.initialObjects)) {
            if (!key || typeof key !== "string") {
                throw new SchemaError("initialObjects 的键必须是非空字符串", ContainerErrorCode.InvalidSchema, { key });
            }
            if (!type || typeof type !== "string") {
                throw new SchemaError(`initialObjects["${key}"] 的类型必须是非空字符串`, ContainerErrorCode.InvalidSchema, { key, type });
            }
        }
        if (schema.dynamicObjectTypes) {
            for (const type of schema.dynamicObjectTypes) {
                if (!type || typeof type !== "string") {
                    throw new SchemaError("dynamicObjectTypes 中的每个类型必须是非空字符串", ContainerErrorCode.InvalidSchema, { type });
                }
            }
        }
    }
}
//# sourceMappingURL=ContainerSchema.js.map